...
class Application (Frame, object):
    ...
    def serialNotification(self):
        ...
        if self.notificationShowed == 0:
            tkinter.messagebox.showerror("Bad Serial Connection", "You are not connected to the Arm USB Port. Please Select a different port")
            self.notificationShowed = 1

    def positionsGet(self, baseP=None, shoulderP=None, elbowP=None, gripperP=None, speedP=None):
        if baseP is None:
            baseP = self.base.get()
        if shoulderP is None:
            shoulderP = self.shoulder.get()
        if elbowP is None:
            elbowP = self.elbow.get()
        if gripperP is None:
            gripperP = self.gripper.get()
        if speedP is None:
            speedP = 25 - self.theSpeed.get()
        return ",".join([str(baseP), str(shoulderP), str(elbowP), str(gripperP), str(speedP)])+"\n"

    def move_it(self, event=None):
        if self.moveIsLocked():
            return
        print("move it")
        ...
        newPositionStr = self.positionsGet()
        self.sendCommand(newPositionStr.encode())
    ...
    def readResponse(self):
        ...
        while empty_count < 1:
            ...
            # print(f"RSP {resp}")
            if resp == b"":   # on an empty byte (no command waiting), increment stop counter
                empty_count += 1
            else:
                message.append(resp)
            reading = (resp != b"")
        response = b''.join(message).decode()
        print(f"RESPONSE {response}") # " from {message}")
        return response
    ...
    def recordArmPos(self):
        ...
        readPosCommand = self.positionsGet()
    ...
    def playback(self):
       ...
       for line in recordFile:
           Count = Count + 1
           self.sendCommand(line.encode())    # send the command to the arduino using another function
       print ('Done')
       recordFile.close()

    def moveIsLocked(self):
        return self.commandBlock
    ...
    def saveFileAs(self):
        ...
        theNewFile = tkinter.filedialog.asksaveasfilename(initialfile='Untitled.txt',defaultextension=".txt",filetypes=[("All Files","*.*"),("Text Documents","*.txt")])
    ...
    def openFile(self):
        ...
        self.currentSequence = tkinter.filedialog.askopenfilename(initialdir = "/",title = "Select file",filetypes = (("txt files","*.txt"),("all files","*.*")))
        print (currentSequence)
        
    def newFile(self):
        #this function created a new .txt file to hold imput commands
        #open a new fle 
        theNewFile = tkinter.filedialog.asksaveasfilename(initialfile='Untitled.txt',defaultextension=".txt",filetypes=[("All Files","*.*"),("Text Documents","*.txt")])	#names the file and sets the location
